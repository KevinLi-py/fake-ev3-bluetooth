...
# from ctypes import addressof, sizeof, struct, ARRAY, UINT8, UINT16
# from socket import socket, SOCK_STREAM
import bluetooth


class socket(bluetooth.BluetoothSocket):
    def read(self, numbytes):
        return self.recv(numbytes)
...
def resolve(name):
    ...
    for addr, device_name in devices:
        if name.strip() == device_name.strip():
            return addr

    else:
        raise ValueError("No device named {!r}".format(name))
...
class RFCOMMServer:
    ...
    def __init__(self, server_address, RequestHandlerClass):
        ...
        self.socket = socket(bluetooth.RFCOMM)
